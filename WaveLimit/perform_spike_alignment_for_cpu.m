...
if size(waveforms,2)>100  %Need a at least 100 spikes on the channel to do the alignment of spike snippets, smaller numbers cause errors and warnings
    ...
    if isnan(alignment_index)  %If there are no waveforms that peak in the window, just return the previous waveforms, AGR 20210528
        ...
        new_waveforms = zeros(n_samples - 2*max_slide_samples, size(waveforms,2));
