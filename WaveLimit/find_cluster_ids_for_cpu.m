...
% Adam Rouse, 4/23/20
% Identifies cluster waveforms on CPU. Looks for spike waveforms that has a
% lot of close neighbors as this signifies a center of a cluster
% Inputs:
...
% - tot_spikes: Total number of spikes for scaling ISI calculations.


% Sort waveforms by ascending timestamps
[timestamps,sort_i] = sort(timestamps,'ascend');
waveforms =  waveforms(:,sort_i);
num_waveforms = size(waveforms,2);  % Number of waveforms

% If there are more than 1000 waveforms, perform PCA for dimensionality reduction
if num_waveforms>1000
    ...
    % Estimate noise level using the first time sample (before threshold crossing)
    noise_level = std(waveforms(1,:));

    % Set thresholds of small and large sigma based on noise level to set a range
    ...
    %sampled well
    %to fill in 501-750 and 751-1000 to get our 1000
    % Select a random subset of 500 waveforms for sampling
    rand_sample = randperm(num_waveforms,500);
    ...
    if length(missed_spikes)>= 250  %If enough spikes outside concentrated areas, then random selection again
    ...
    % Filter out waveforms with fewer than 50 neighbors for clustering since these can't be cluster centers
    sample_dist([false(500,1);true(250,1)] & sum(sample_dist(:,:,1)<large_sigma,2)<50,:,:) = NaN;
    ...
    % Identify spikes that are poorly clustered (missed spikes)
    missed_spikes = find(sum(sample_dist(:,:,1),1)<1);
    if length(missed_spikes)>= 250  %If enough spikes outside concentrated areas, then look once more for cluster center
        % Randomly select missed spikes for clustering
        rand_sample(751:1000) = missed_spikes(randperm(length(missed_spikes),250));
    else
        % Otherwise, Sort spikes by their distance to nearest clusters
        [~,sort_spikes_i] = sort(sum(sample_dist(:,:,1),1),'ascend');
    ...
    % Handle cases where the number of units exceeds the allowed maximum, default max is 20
